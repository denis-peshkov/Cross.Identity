...
public class ExternalLoginComplete_StepTests
{
    ...
    public async Task GivenSuccessfulLogin_WhenExecuteAsync_ThenIssuesTokensAsync()
    {
        ...
        _jwtTokenService
            .Setup(j => j.GenerateAccessTokenAsync(userId, It.IsAny<Guid>(), It.IsAny<List<string>>(), It.IsAny<List<Claim>>(), ClientContext.Empty, It.IsAny<CancellationToken>()))
            .ReturnsAsync("access-token");
        _jwtTokenService
            .Setup(j => j.GenerateRefreshTokenAsync(userId, It.IsAny<Guid>(), It.IsAny<List<Claim>>(), ClientContext.Empty, It.IsAny<CancellationToken>()))
