...
public class CollectForm_StepTests
{
    ...
    [Test]
    [Category(TestCategory.UNIT)]
    public async Task GivenOptionalFieldAbsent_WhenExecuteAsync_ThenStoresNullAsync()
    {
        var schema = new FormSchema("collectForm", new[]
        {
            new FieldDescriptor("Email", FieldTypeEnum.Email, Required: true, Max: 256),
            new FieldDescriptor("UserAgent", FieldTypeEnum.String, Required: false, Max: 512),
        });
        var step = new CollectFormStep
        {
            Kind = "collectForm",
        ...
        result.Status.Should().Be(StepStatusEnum.Ok);
        bag.Get<string>("collectForm.Email").Should().Be("a@b.c");
        bag.Has("collectForm.UserAgent").Should().BeTrue();
        bag.Get<string?>("collectForm.UserAgent").Should().BeNull();
    }
