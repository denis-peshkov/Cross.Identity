...
public class ExternalLoginUnlink_StepTests
{
    ...
    public async Task GivenProviderAndUserId_WhenExecuteAsync_ThenUnlinksAndSetsFlagAsync()
    {
        ...
        _externalLoginService
            .Setup(s => s.UnlinkAsync("Google", userAccountId, HostSuppliedClientContext.Empty, It.IsAny<CancellationToken>()))
        ...
        _externalLoginService.Verify(
            s => s.UnlinkAsync("Google", userAccountId, HostSuppliedClientContext.Empty, It.IsAny<CancellationToken>()),
            Times.Once);
    }
}
