...
internal static class TokenTestHelpers
{
    ...
    public static async Task<bool> IsRefreshTokenActiveAsync(IdentityContext context, string refreshToken)
    {
        var hash = RefreshTokenHash(refreshToken);
        var entity = await context.RefreshTokens.AsNoTracking().FirstOrDefaultAsync(x => x.TokenHash == hash).ConfigureAwait(false);
